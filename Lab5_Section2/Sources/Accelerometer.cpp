...
/**************************************************/
int constexpr MAX =5000;
int constexpr MIN = -5000;
int constexpr LEVEL_MAX =8;
 ...
 */
void printTheCurrentLevel(unsigned level)
{
	console.write("The current level is: ").writeln(level);
}
int16_t report() {
   int accelStatus;
...
//         write(", aZ=").writeln(accelZ);
	//printTheCurrentLevel(calculateTheDegree(accelY));
	return accelY;
}
unsigned calculateTheDegree()
{
	int16_t accelY=report();
		accelY+=MAX;
		return (LEVEL_MAX-1-(accelY*LEVEL_MAX/(MAX-MIN)));
}
void initializeTheAccelerometer()
{
